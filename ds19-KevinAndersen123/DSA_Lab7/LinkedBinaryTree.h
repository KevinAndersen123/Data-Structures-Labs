...
inline void LinkedBinaryTree<T, Iterator>::deleteItem(Iterator & position)
{
	...
	//leaf node
	else if (position.node()->leftChild() == nullptr && position.node()->rightChild() == nullptr)
	{
		Iterator iterParent = parent(position);
		if (iterParent.node()->leftChild() == position.node())
		{
			iterParent.node()->setLeftChild(nullptr);
		}
		else
		{
			iterParent.node()->setRightChild(nullptr);
		}
	...
	else if ((position.node()->leftChild() != nullptr && position.node()->rightChild() != nullptr))
	{
		...
		while (iter.node()->rightChild() != nullptr)
		{
			iter = rightChild(iter);
		}

		position.node()->setElement(iter.node()->element());
	...
	else
	{
		...
		{
			position.node()->setElement(position.node()->leftChild().node()->element());
			delete position.node()->leftChild().node();
			position.node()->setLeftChild(nullptr);
		}
		else
		{
			position.node()->setElement(position.node()->rightChild().node()->element());
			delete position.node()->rightChild().node();
			position.node()->setRightChild(nullptr);
		}
	}
